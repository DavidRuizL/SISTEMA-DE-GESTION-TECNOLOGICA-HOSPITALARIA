...
if(isset($_GET["insertar"])){
        ...
        $nom_resp=$data->nom_resp;
            if(($nombre!="")&&($marca!="")&&($modelo!="")&&($serial!="")&&($doc_resp!="")&&($nom_resp!="")){        
                $sqlActivosb = mysqli_query($conexionBD,"INSERT INTO activos(nombre,marca,modelo,serial,doc_resp,nom_resp) VALUES('$nombre','$marca','$modelo','$serial','$doc_resp','$nom_resp') ");
                echo json_encode(["success"=>1]);
            }
        exit();
...
if (isset($_GET["borrar_resp"])){
    $sqlPacientes = mysqli_query($conexionBD,"DELETE FROM responsables WHERE id=".$_GET["borrar_resp"]);
    if($sqlResponsable_){
        echo json_encode(["success"=>1]);
        exit();
    }
